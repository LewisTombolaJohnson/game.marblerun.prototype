...
import { ClientToServerEvents, ServerToClientEvents, RaceSnapshot, FinishResult, TrackData, TrackObstacle, TrackObstacleBox } from '../../shared/events';

export interface RaceInstance { start: () => void; }
...
export function createRace(io: Server<ClientToServerEvents, ServerToClientEvents>, lobby: InternalLobby): RaceInstance {
  ...
  function randomId(prefix: string, i: number) { return `${prefix}_${i}`; }
  ...
  const nominalTrackLength = 160; // base for initial platform spacing reference
  ...
  // Determine wall span: from a bit above spawn (spawn y ≈ 2) down past finish line.
  const spawnY = 2;
  const topMargin = 12; // extend above spawn so early lateral movement is contained
  const bottomExtra = 15; // extend below finish line to catch overshoot
  const yTop = spawnY + topMargin; // higher (less negative / positive) world y
  const yBottom = finishLineY - bottomExtra; // further downwards (more negative)
  const wallCenterY = (yTop + yBottom) / 2;
  const wallLength = Math.abs(yTop - yBottom);
  obstacles.push({ id: 'wallL', type: 'box', position: { x: -halfWidth, y: wallCenterY }, size: { x: wallThickness, y: wallLength } });
  obstacles.push({ id: 'wallR', type: 'box', position: { x: halfWidth, y: wallCenterY }, size: { x: wallThickness, y: wallLength } });
    ...
    marbles.forEach(m => {
      if (!m.finished && m.y < finishThreshold) {
        ...
        const result: FinishResult = {
          playerId: m.playerId,
          name: player ? player.name : m.playerId.slice(0,4),
          timeMs: m.finishTime,
  ...
  function handleCollisions() {
    if (!trackData) return;
    const obstacles = trackData.obstacles as TrackObstacleBox[];
    obstacles.forEach(o => {
        ...
        if (distSq <= radius*radius) {
          // Collision with side or corner
            ...
            if (distSq === 0) {
              ...
              if (penX < penY) { nx = localX > 0 ? 1 : -1; ny = 0; }
              else { nx = 0; ny = localY > 0 ? 1 : -1; }
            } else {
